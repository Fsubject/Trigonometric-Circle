...
class Window:
    def __init__(self):
        ...
        self.c_center = self.circle.get_center()
        self.c_radius = self.circle.get_radius()
    ...
    def run(self) -> None:
        while self.running:
            ...
            for idx, point in enumerate(self.c_angles):
                ...
                pygame.draw.line(self.window, BLACK if idx != self.angle_i else RED, self.c_center, point[0], 3)

            if self.is_placing:
                x, y = self.circle.get_xy(mouse_pos)
                pygame.draw.line(self.window, GREEN, self.c_center, (x, y), 3)
